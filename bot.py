    ...
    try:
        ...
        while True:
            ...
            for i, text in enumerate(shuffled_posts):
                if i > 0:
                    await asyncio.sleep(10)  # 5 hours between posts (18000 seconds)
                try:
                    ...
                    # Always send the button as a separate message
                    await bot.send_message(chat_id, "", reply_markup=reply_markup)

                    # After sending post 3 or 4, send lesson opening for the current unopened lesson
                    if post_index in [3, 4]:
                        stage = get_stage(chat_id)
                        if stage < 3:
                            current_lesson = stage
                            await bot.send_message(chat_id, f"Напоминаю: урок {current_lesson} всё ещё ждёт тебя.👇", reply_markup=kb_open(current_lesson), parse_mode=ParseMode.HTML)
                except TelegramForbiddenError:
                    break
                except Exception as e:
