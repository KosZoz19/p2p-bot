...
У многих есть страхи. Страхи что не выйдет, что не получится, что обманут и т.д. И бороться с этими всеми страхами можно только лишь с <b>подходом к Р2Р с правильной стороны и с хорошим наставником.</b>
...
<blockquote>Вся суть Р2Р это начать с малого, просто чтобы понять и попробовать. А со временем у тебя в руках оказывается инструмент финансовой свободы.Здесь нет потолка, нет ограничений — всё зависит только от того, сколько ты готов вкладывать усилий.

И да, именно поэтому я считаю P2P профессией будущего. Люди всегда будут обменивать: вчера это было золото на серебро, сегодня крипта на фиат, завтра — ещё что-то новое.Но суть одна: спрос есть и будет всегда.</blockquote>
...
Но реальность оказалось куда другой. Со временем я понял что Р2Р это далеко не темка а прогнозируемый бизнес на дистанции который дает хорошие результаты. И результат не заставил себя ждать, через какое-то время работы я купил себе Mercedes 

P2P  дало мне свободу. И не ту лживую как какие-то темки или профессии, а правдивую. Где нету начальника, привязанности к месту и потолка дохода. Благодаря Р2Р я могу не переживать о завтрашнем дне и что не менее важно провести его как в Польше так и в Испании

<blockquote>Здесь я научился ценить эту свободу. Свободу выбора, а не обязанность работать на кого-то. Не ту, о которой любят красиво говорить, а настоящую — когда ты не зависишь от зарплаты в конце месяца, не смотришь на начальника, не переживаешь, что завтра тебя могут уволить.
P2P дало мне уверенность, что у меня всегда будет доход. Потому что люди всегда будут обменивать деньги. Раньше это могли быть золотые и серебрянные монеты, сейчас же криптовалюта и деньги</blockquote>
...
async def send_course_posts(chat_id: int):
    ...
    try:
        await asyncio.sleep(18000)  # Wait 10 seconds before starting
        while True:
            sent_posts = set()
            ...
            for i, text in enumerate(shuffled_posts):
                if i > 0:
                    await asyncio.sleep(18000)  # 5 hours between posts (18000 seconds)
                try:
